...
*
*/
bool convertMatToIntArr3D(vector<Mat> images, int*** result, int count, int row, int col) {
	if (!result) {
		fprintf(stderr, "Result array is null.\n");
		return false;
	}
	...
	for (int k = 0; k < count; k++) {
		if (images[k].empty()) {
			fprintf(stderr, "Image %d is empty.\n", k);
			return false;
		}

		if (images[k].rows != row || images[k].cols != col) {
			fprintf(stderr, "Image %d dimensions do not match.\n", k);
			return false;
		}

		for (int i = 0; i < row; i++) {
			for (int j = 0; j < col; j++) {
				result[k][i][j] = static_cast<int>(images[k].at<uchar>(i, j));
			}
		}
	}
	...
	const int count, const int row, const int col, const int row_output, const int col_output) 
{
	...
	float time_memcopy_images = 0.0f;
	float time_memcopy_filter = 0.0f;
	float time_memcopy_result_in = 0.0f;
	float time_memcopy_result_out = 0.0f;
	...
	extent = make_cudaExtent(row * sizeof(int), count, col);
	...
	gridDim = dim3(1, grid_y, 1);
	blockDim = dim3(1, block_y, 1);
	cudaEventRecord(start);
...
Error:
	...
	if (dev_ptr.ptr) cudaFree(dev_ptr.ptr);
	if (dev_ptr_output.ptr) cudaFree(dev_ptr_output.ptr);
...
)
{
	...
	num_blocks = count / threads_per_block + 1;
...
Error:
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
	if (image_arrptr.ptr) cudaFree(image_arrptr.ptr);
	if (new_image_arrptr.ptr) cudaFree(new_image_arrptr.ptr);
