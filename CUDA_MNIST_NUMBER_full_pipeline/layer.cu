...
bool Layer::saveWeights(const char* weightFile, const char* biasFile) {
...
bool Layer::loadWeights(const char* weightFile, const char* biasFile) {
    float* h_weight = new float[M*N];
    float* h_bias = new float[N];
    
    // Load weights
    FILE* fw = fopen(weightFile, "rb");
    if (!fw) {
        delete[] h_weight;
        delete[] h_bias;
        return false;
